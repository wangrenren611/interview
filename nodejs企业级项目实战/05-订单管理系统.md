...
1. **购物车管理**：商品添加、数量修改、删除等操作
2. **订单创建**：从购物车生成订单、订单信息确认
3. **订单支付**：订单支付处理、支付状态更新
4. **订单状态管理**：订单状态流转、物流跟踪
5. **订单查询**：订单列表、订单详情查询
6. **订单操作**：订单取消、退货退款等操作
...
class Order extends BaseModel {
  ...
  // 触发状态变更事件
  async emitStatusChangeEvent(newStatus, options = {}) {
    const event = {
      orderId: this.id,
  ...
  // 预留库存
  async reserveInventory(options = {}) {
    const items = await this.getOrderItemList();
  ...
  // 释放库存
  async releaseInventory(options = {}) {
    const items = await this.getOrderItemList();
...
#### 2.2.1 订单商品模型实现
```javascript
// src/app/models/order-item.model.js
const { DataTypes } = require('sequelize');
const BaseModel = require('./base.model');
...
#### 2.3.1 状态历史模型实现
```javascript
// src/app/models/order-status-history.model.js
const { DataTypes } = require('sequelize');
const BaseModel = require('./base.model');
...
```javascript
// src/app/models/cart-item.model.js
const { DataTypes } = require('sequelize');
const BaseModel = require('./base.model');
