...
### 1.0 TypeScript在支付系统中的优势

在支付系统集成中，TypeScript提供了强大的类型安全保障，特别是在处理复杂的支付流程和第三方API集成时：
...
1. **多支付渠道支持**：支付宝、微信支付、银联等主流支付方式，类型安全的渠道管理
2. **支付流程管理**：支付创建、支付处理、支付回调，强类型的流程控制
3. **支付状态同步**：实时更新订单支付状态，类型化的状态机
4. **退款处理**：订单退款申请和处理，类型安全的退款流程
5. **对账管理**：支付流水对账和异常处理，类型化的对账逻辑
6. **安全防护**：支付安全、防欺诈、风控管理，类型安全的验证机制
...
interface PaymentInstance extends Model<PaymentAttributes, PaymentCreationAttributes>, PaymentAttributes {
  ...
  // 关联方法
  getRefundList(options?: any): Promise<Refund[]>;
  createRefund(data: any, options?: any): Promise<Refund>;
}

// 支付渠道显示名称映射
const CHANNEL_DISPLAY_NAMES: Record<PaymentChannel, string> = {
  'alipay': '支付宝',
  'wechat_pay': '微信支付',
  'unionpay': '银联支付',
  'apple_pay': 'Apple Pay',
...
// 支付状态显示名称映射
const STATUS_DISPLAY_NAMES: Record<PaymentStatus, string> = {
  'pending': '待支付',
  'paid': '已支付',
  'failed': '支付失败',
  'cancelled': '已取消',
...
class Payment extends BaseModel<PaymentAttributes, PaymentCreationAttributes> {
  declare id: number;
  declare order_id: number;
  declare payment_no: string;
  declare channel: PaymentChannel;
  declare amount: number;
  declare currency: string;
  declare status: PaymentStatus;
  declare third_party_trade_no?: string;
   ...
   */
  getChannelDisplayName(): string {
    return CHANNEL_DISPLAY_NAMES[this.channel] || this.channel;
  }
  ...
  async getRefundableAmount(): Promise<number> {
    ...
    const refunds = await this.getRefundList({
      where: { status: 'success' }
    });

    const totalRefunded = refunds.reduce((sum: number, refund: Refund) => sum + refund.amount, 0);
  ...
  async updatePaymentStatus(
    ...
    if (thirdPartyTradeNo) {
      updateData.third_party_trade_no = thirdPartyTradeNo;
    }
...
import BaseModel from './base.model';
import { RefundAttributes, RefundCreationAttributes } from '../types/payment.types';

// 退款记录模型接口
interface RefundInstance extends Model<RefundAttributes, RefundCreationAttributes>, RefundAttributes {
  // 实例方法
  getStatusDisplayName(): string;
...
// 退款状态显示名称映射
const REFUND_STATUS_DISPLAY_NAMES: Record<RefundStatus, string> = {
  'pending': '待处理',
  'processing': '处理中',
  'success': '退款成功',
  'failed': '退款失败'
...
class Refund extends BaseModel<RefundAttributes, RefundCreationAttributes> {
  declare id: number;
  declare payment_id: number;
  declare refund_no: string;
  declare amount: number;
  declare reason: string;
  declare status: RefundStatus;
  declare third_party_refund_no?: string;
   ...
   */
  getStatusDisplayName(): string {
    return REFUND_STATUS_DISPLAY_NAMES[this.status] || this.status;
  }
...
#### 3.1.1 支付宝支付服务
```typescript
// src/app/services/payment/alipay.service.ts
import AlipaySdk from 'alipay-sdk';
import AlipayFormData from 'alipay-sdk/lib/form';
import { Payment } from '../../models';
import { config } from '../../../config';
import { CreatePaymentRequest, CreatePaymentResponse, PaymentCallbackData } from '../../types/payment.types';
...
#### 3.2.1 微信支付服务
```typescript
// src/app/services/payment/wechat-pay.service.ts
import WechatPay from 'wechatpay-node-v3';
import { Payment } from '../../models';
import { config } from '../../../config';
import { CreatePaymentRequest, CreatePaymentResponse } from '../../types/payment.types';
...
class WechatPayService {
  ...
  constructor() {
    this.config = {
      ...
      sandbox: config.wechatPay.sandbox || false
    };

    this.wechatPay = new WechatPay(this.config);
  }
   ...
   * @returns 支付响应
   */
  async createPayment(order: Order, options: CreatePaymentRequest = {}): Promise<CreatePaymentResponse> {
    const payment = await Payment.create({
      order_id: order.id,
      user_id: order.user_id,
      channel: 'wechat_pay',
