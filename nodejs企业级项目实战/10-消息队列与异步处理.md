...
1. **异步处理**：将耗时操作放入队列后台处理，提高响应速度
2. **系统解耦**：降低系统组件间的耦合度，提高可维护性
3. **流量削峰**：在高并发场景下平滑处理请求
4. **可靠性保障**：通过持久化和重试机制确保任务执行
...
// src/queues/order-processing.queue.js
...
// 创建订单处理队列
const orderProcessingQueue = QueueManager.createQueue('order-processing', {
  limiter: {
...
```javascript
// src/queues/data-processing.queue.js
const QueueManager = require('./queue-manager');
...
// 创建数据处理队列
const dataProcessingQueue = QueueManager.createQueue('data-processing', {
  limiter: {
    max: 10, // 每分钟最多处理10个任务
    duration: 60000
  },
