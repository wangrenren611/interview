...
1. **商品信息管理**：商品的增删改查操作
2. **商品分类管理**：商品分类的层级结构管理
3. **商品属性管理**：商品规格、属性的定义和管理
4. **库存管理**：商品库存的实时更新和预警
5. **商品搜索**：基于关键词、分类、价格等条件的商品搜索
6. **商品推荐**：热门商品、新品推荐等
...
class Product extends BaseModel {
  // 检查库存是否充足
  async hasEnoughStock(quantity, variantId = null) {
...
#### 2.2.1 分类模型实现
```javascript
// src/app/models/category.model.js
const { DataTypes } = require('sequelize');
const BaseModel = require('./base.model');
...
#### 2.3.1 库存模型实现
```javascript
// src/app/models/inventory.model.js
const { DataTypes } = require('sequelize');
const BaseModel = require('./base.model');

class Inventory extends BaseModel {
  // 增加库存
  async increaseQuantity(quantity, options = {}) {
    if (quantity <= 0) {
