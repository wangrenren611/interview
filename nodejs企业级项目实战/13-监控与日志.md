...
1. **主动发现问题**：通过实时监控及时发现系统异常
2. **快速定位故障**：提供详细的指标和日志帮助快速定位问题根源
3. **性能优化指导**：通过历史数据分析指导系统性能优化
4. **容量规划支持**：基于监控数据进行资源容量规划
...
const infrastructureMetrics = {
  ...
  // 磁盘IO
  diskIO: {
    readBytes: 1024000, // 读取字节数(bytes/s)
    writeBytes: 512000, // 写入字节数(bytes/s)
    readOps: 100, // 读操作次数(ops/s)
    writeOps: 50 // 写操作次数(ops/s)
  },
...
const { httpRequestTotal, httpRequestDuration } = require("../metrics/prometheus");
